...
def train_single_model(model_key: str, config: dict, device: torch.device):
    ...
    try:
        ...
        logger.info(f"  │  BLEU-1:            {all_metrics['bleu_1']:<24.2f} │")
        logger.info(f"  │  BLEU-2:            {all_metrics['bleu_2']:<24.2f} │")
        logger.info(f"  │  BLEU-4:            {all_metrics['bleu_4']:<24.2f} │")
        logger.info("  ├──────────────────────────────────────────────┤")
        logger.info(f"  │  ROUGE-1:           {all_metrics['rouge_1']:<24.2f} │")
        logger.info(f"  │  ROUGE-2:           {all_metrics['rouge_2']:<24.2f} │")
        logger.info(f"  │  ROUGE-L:           {all_metrics['rouge_l']:<24.2f} │")
        logger.info("  ├──────────────────────────────────────────────┤")
        logger.info(f"  │  BERTScore P:       {all_metrics.get('bertscore_precision', 0):<24.2f} │")
        logger.info(f"  │  BERTScore R:       {all_metrics.get('bertscore_recall', 0):<24.2f} │")
