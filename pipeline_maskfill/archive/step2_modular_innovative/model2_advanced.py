...
class PrivacyAttentionHead(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        # Privacy importance score per token: sigmoid → [0, 1]
        privacy_scores = torch.sigmoid(self.privacy_proj(attn_out)).squeeze(-1)
        return privacy_scores, attn_weights
...
class MultiTaskNERModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None,
        ...
        # Privacy scores
        privacy_scores, attn_weights = self.privacy_head(hidden, attention_mask)
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                if valid.any():
                    ...
                    privacy_loss = F.binary_cross_entropy(
                        privacy_scores[valid], privacy_target[valid])
        ...
        # Attach extra attributes for our custom trainer
        output.privacy_scores = privacy_scores
        output.privacy_loss = privacy_loss
        return output
...
class FocalTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss = focal_loss
        if out.privacy_loss is not None:
            loss = loss + 0.3 * out.privacy_loss
