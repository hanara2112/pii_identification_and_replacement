...
from common import (
    ...
    GENERALIZER, plot_evaluation_summary, _find_last_checkpoint,
...
def run_semantic_pipeline(half_a, half_b, test_set, lang_col):
    ...
    para_adapter = os.path.join(para_dir, "adapter_config.json")
    if os.path.exists(para_adapter):
        log.info(f"Paraphraser already trained — loading from {para_dir}")
        ...
        base = AutoModelForSeq2SeqLM.from_pretrained(
            CFG.PARAPHRASER_BASE, quantization_config=bnb, device_map="auto")
        para_model = _PeftModel.from_pretrained(base, para_dir)
    else:
        # Create training data only if we need to train
