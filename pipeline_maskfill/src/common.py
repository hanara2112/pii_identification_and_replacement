...
def restore_checkpoints_from_input():
    """Copy model output dirs from Kaggle input mounts to /kaggle/working/outputs.

    When you 'Save Version' on Kaggle, the output goes to /kaggle/working/.
    To resume, add the previous run's output as an input dataset — it mounts
    somewhere under /kaggle/input/. This function finds known model directories
    (model1_baseline, model2_advanced, etc.) and copies their ENTIRE contents
    (checkpoints, final model files, tokenizer, etc.) to the working directory
    so that (a) completed stages can be skipped and (b) the trainer can resume
    from checkpoints.
    """
    if not IS_KAGGLE:
        return
    ...
    KNOWN_MODEL_DIRS = {
        "model1_baseline", "model2_advanced",
        "model3_rephraser", "model4_semantic",
    }
    ...
    for root, dirs, _files in os.walk(input_root):
        # Check if any child dir is a known model dir
        for d in list(dirs):
            if d not in KNOWN_MODEL_DIRS:
                continue
            src = os.path.join(root, d)
            dst = os.path.join(BASE_DIR, "outputs", d)
            found_dirs.append(d)

            # Merge: copy every file that doesn't already exist in dst
            for sub_root, sub_dirs, sub_files in os.walk(src):
                ...
                for f in sub_files:
                    ...
                    if not os.path.exists(dst_file):
                        shutil.copy2(os.path.join(sub_root, f), dst_file)
                        copied_files += 1

            # Don't descend further into this model dir from the outer walk
            dirs.remove(d)

    if found_dirs:
        log.info(f"  Restored {copied_files} file(s) from input datasets "
                 f"for: {', '.join(sorted(found_dirs))}")
    else:
        log.info("  No model output dirs found in input datasets")
...
def _find_last_checkpoint(output_dir):
    """Return path to last HF Trainer checkpoint in output_dir, or None."""
    ...
    log.info(f"  Found checkpoint: {path} — resuming training")
    return path
...
def train_seq2seq(model, tok, train_ds, val_ds, output_dir,
    ...
    args = Seq2SeqTrainingArguments(
        ...
        logging_steps=100, save_total_limit=1, report_to="none",
        dataloader_num_workers=2, dataloader_pin_memory=True,
    )
    ckpt = _find_last_checkpoint(output_dir)
    ...
    # Enable generation for final eval only (skipped during training for speed)
