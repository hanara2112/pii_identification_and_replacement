...
from common import (
    ...
    run_curated_eval, cleanup_gpu, _find_last_checkpoint,
...
def run_advanced(half_a, half_b, test_set, lang_col):
    ...
    # HF Trainer may save as model.safetensors; custom save_pretrained uses pytorch_model.bin
    censor_saved_st = os.path.join(censor_dir, "model.safetensors")
    censor_saved_pt = os.path.join(censor_dir, "pytorch_model.bin")
    censor_already_trained = os.path.exists(censor_saved_st) or os.path.exists(censor_saved_pt)
    if censor_already_trained:
        log.info(f"Censor already trained — loading from {censor_dir}")
        censor_tok = AutoTokenizer.from_pretrained(censor_dir)
        censor_model = MultiTaskNERModel(CFG.CENSOR_ADV, NUM_LABELS)
        if os.path.exists(censor_saved_st):
            from safetensors.torch import load_file
            censor_model.load_state_dict(load_file(censor_saved_st))
        else:
            censor_model.load_state_dict(torch.load(censor_saved_pt, map_location="cpu"))
        censor_model = censor_model.float().to(DEVICE)
        log.info("  Loaded MultiTaskNERModel from saved weights")
    else:
    ...
    # ── DP-SGD refinement (skip if censor was loaded from disk) ──
    if not censor_already_trained:
        tr, va = make_train_val(half_a)
    ...
    halluc_adapter = os.path.join(halluc_dir, "adapter_config.json")
    if os.path.exists(halluc_adapter):
        log.info(f"Hallucinator already trained — loading from {halluc_dir}")
        ...
        base = AutoModelForSeq2SeqLM.from_pretrained(
            CFG.HALLUC_ADV, quantization_config=bnb, device_map="auto")
        halluc_model = PeftModel.from_pretrained(base, halluc_dir)
    else:
